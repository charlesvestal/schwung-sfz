...
char *convert_dspreset_to_xsynth_sfz(const char *path,
    ...
    while ((scan = strstr(scan, "<group")) != NULL) {
        ...
        /* MOVE FORK / 2026-05-19: DS `<group polyphony="N">` support.
         * Currently only N=1 (monophonic group) maps cleanly — we emit
         * a unique symmetric choke `group=<id> off_by=<id>` on every
         * region of that group so a new note kills any prior note from
         * the same group. N>1 would need real per-group polyphony
         * tracking in xsynth (which we don't have); for those we just
         * defer to the channel-level cap.
         *
         * ID range: 200..255 so polyphony-1 IDs never collide with the
         * tag-based choke IDs (which start at 1 and rarely exceed 32). */
        static int  ds_mono_next_id = 200;
        if (group_idx == 0) {
            ds_choke_count = 0;
        ...
        /* MOVE FORK / 2026-05-19: DS `<group polyphony="1">` →
         * symmetric SFZ choke (group=<unique> off_by=<unique>) so the
         * group goes monophonic. Allocated per-group from the 200+
         * ID range so they don't collide with tag-based choke IDs. */
        {
            char poly_attr[32];
            xml_get_attr(tag_buf, "polyphony", poly_attr, sizeof(poly_attr));
            if (poly_attr[0] && atoi(poly_attr) == 1
                    && ds_mono_next_id < 255) {
                int mono_id = ds_mono_next_id++;
                ds_choke_group_id = mono_id;
                ds_choke_off_by   = mono_id;
            }
        }
